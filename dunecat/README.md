...
A thin Python library + CLI on top of
[`metacat-client`](https://fermitools.github.io/metacat/) tailored to the
workflows of finding datasets, inspecting metadata, and filtering files by
run, run range, date, or arbitrary metadata.

See the repo root [`README.md`](../README.md) for install and authentication;
this page covers only the command surface.

## Authenticate

```bash
uv run dunecat login                   # uses METACAT_USER / METACAT_AUTH_METHOD from .env
uv run dunecat login -u alice -m x509  # one-off override
```

Wraps `metacat auth login` so you don't have to type the server URLs.
Tokens land in `~/.token_library` and last one week.

## Conventions

All commands write payload to stdout and progress/errors to stderr. List-shaped
output is one identifier per line by default; `--json` emits JSONL (one object
per line) so the stream stays memory-bounded. `dataset show` prints a `rich`
table for humans and a single-line JSON object with `--json`.
...
`YYYYMMDDTHHMMSS` match in the file's name portion of the DID, parsed as UTC.
Override with `--filename-time-regex PATTERN` and
`--filename-time-format STRFTIME` for other naming conventions.

To prevent client-side date filters from chewing through huge candidate sets,
the streaming pipeline aborts (exit 1) if more than
`--date-range-max-candidates` files arrive before the post-filter
(default 10000). Narrow with `--runs`, `--run-range`, `--namespace`, or
`--meta` first.
...
Given a file DID (a single file's `namespace:name`), list the datasets it
belongs to. A file commonly lives in several datasets: an umbrella like
`dune:all`, a per-batch production set, and curated subsets.
...
MQL syntax errors from the server surface with the server's full message on
stderr and exit 1.

## Exit codes

- `0` — success
- `1` — query error, no matches when one was expected, or candidate-cap exceeded
- `2` — auth error (token missing or expired); see the repo root README for
  the `metacat auth login` command
